...
const schema= [
    ...
    body('userLastName').isLength({min:3,max:30}).withMessage('The last name must contain a minimum of 3 and a maximum of 30 characters'),
    body('userLastName').exists().withMessage('You need to enter the last name'),
    body('userLastName').isAlpha('es-ES',{ignore: ' '}).trim().escape().withMessage('The last name must only contain letters and spaces'),    
